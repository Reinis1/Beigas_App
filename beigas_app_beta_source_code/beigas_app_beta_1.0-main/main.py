...
# Nepieciešams izveidot logu pirms tiek importēti citi "Kivy" līdzekļi, lai novērstu kļūdas
...
try:
    ...
    data = {'done_setup': False, 'saraksts_11_DIT': default_saraksts}
...
class MainWidget(Screen):  # Main class -------------------------------------------------------------------------
    ...
    def get_current_lesson_index(self):
    ...
    def update(self, dt):
        ...
        if Global.what_is not in ('start', 'setup'):
            Global.lesson_index = self.get_current_lesson_index()
    ...
    def update_current_lesson(self):
        if Global.what_is == "lesson":
            ...
            end = self.get_min_until_lesson_end(Global.lesson_index)
    ...
    def get_min_until_lesson_end(self, lesson_index):
        end_time = datetime.strptime(f"{stundu_laiki[lesson_index][1][0]}:"
                                     f"{stundu_laiki[lesson_index][1][1]}", "%H:%M").time()
    ...
    def min_until_bb_end(self, lesson_index):
        end_time = datetime.strptime(f"{stundu_laiki[lesson_index + 1][0][0]}:"
                                     f"{stundu_laiki[lesson_index + 1][0][1]}", "%H:%M").time()
        end_time = datetime.combine(datetime.today(), end_time)
        time_difference = end_time - datetime.now()
        return ((time_difference.seconds % 3600) // 60)
...
class Break(Screen):  # BREAK SCREEN WIDGET --------------------------------------------
    ...
    def min_until_break_end(self):
        end_time = datetime.strptime(
            f"{stundu_laiki[Global.break_index + 1][0][0]}:{stundu_laiki[Global.break_index + 1][0][1]}",
            "%H:%M").time()
        end_time = datetime.combine(datetime.today(), end_time)
        time_difference = end_time - datetime.now()
...
class Free(Screen):
    ...
    def next_lesson_starts_in(self):
        current_time = datetime.now().time()
        end_time = datetime.strptime(f"{stundu_laiki[0][0][0]}:{stundu_laiki[0][0][1]}",
                                     "%H:%M").time()
...
class Start(Screen):
    # needed to init start screen in build function
    pass
